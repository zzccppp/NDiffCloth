...
class Triangle {
  public:
    ...
    double evaluateEnergy(const VecXd &x_new);
    ...
    Vec9d dArea_dxnew(const VecXd &x) const {
        ...
        dv1v2_dx0x1x2.block<3, 3>(v1_row, x0_col) =
            -Mat3x3d::Identity();   // dv1_dx0
        dv1v2_dx0x1x2.block<3, 3>(v2_row, x0_col) =
            -Mat3x3d::Identity();   // dv2_dx0

        dv1v2_dx0x1x2.block<3, 3>(v1_row, x1_col) =
            Mat3x3d::Identity();                               // dv1_dx1
        dv1v2_dx0x1x2.block<3, 3>(v2_row, x1_col).setZero();   // dv2_dx1

        dv1v2_dx0x1x2.block<3, 3>(v1_row, x2_col).setZero();   // dv1_dx1
        dv1v2_dx0x1x2.block<3, 3>(v2_row, x2_col) =
            Mat3x3d::Identity();   // dv2_dx1
    ...
    void addConstraint(std::vector<Triplet> &tri, int &c_idx, bool withWeight);

    VecXd project(const VecXd &x_vec) const;

    void projectBackward(const VecXd &x_vec, TripleVector &triplets);

    void projectBackwardPrecompute(const VecXd &x_vec);

    VecXd dp_dk(const VecXd &x_vec) const;
