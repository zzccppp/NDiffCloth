...
using simulation::Triangle;
using utils::insertIntoTriplets;
using utils::kronecker;
...
Mat3x2d
Triangle::getDeformationGradient(const VecXd &x_vec) const {
    Mat3x2d p;
...
double
Triangle::evaluateEnergy(const Mat3x2d &F, const VecXd &x_new) {
    switch (energyType) {
    case QUADRATIC:
...
double
Triangle::evaluateEnergy(const VecXd &x_new) {
    return evaluateEnergy(getDeformationGradient(x_new), x_new);
}

Vec9d
Triangle::stretchingForce(const VecXd &x_new) {
    ...
    case NON_QUADRATIC: {
        ...
        Vec3d dE_deps = Vec3d(k[0] * e_uu + k[1] * e_vv,
                              k[1] * e_uu + k[2] * e_vv, 2 * k[3] * e_uv);
...
Vec9d
Triangle::dfi_dk(const VecXd &x_new) {
    Mat3x2d F = getDeformationGradient(x_new);
    Vec3d F_c0 = F.col(0), F_c1 = F.col(1);   // should equal Du*mat_to_vec(X)
    ...
    case QUADRATIC: {
        ...
        dfi_dk_buffer = stretchingForce(x_new) /
                        (constrainWeightSqrt * constrainWeightSqrt);
...
Triangle::stretchingForceAndJacobianCheck() const {
    ...
    const Vec3d &xu = F.col(0),
                &xv = F.col(1);   // should equal Du*mat_to_vec(X)
    ...
    Mat9x9d hess_e2 =
        k[0] * (outer(fuu_2, fuu_2) + std::fmax(e_uu, 0.) * Du.transpose() *
                                          Du)   // Jacobian of force
        + k[2] * (outer(fvv_2, fvv_2) +
                  std::fmax(e_vv, 0.) * Dv.transpose() * Dv) +
        k[1] *
            (outer(fuu_2, fvv_2) + std::fmax(e_uu, 0.) * Dv.transpose() * Dv +
...
Mat9x9d
Triangle::stretchingHessian(const VecXd &x_new) const {
    ...
    case NON_QUADRATIC: {
        ...
        Vec3d F_c0 = F.col(0),
              F_c1 = F.col(1);   // should equal Du*mat_to_vec(X)
        ...
        Mat9x9d hess_e =
            k[0] * (outer(fuu, fuu) + std::fmax(e_uu, 0.) * Du.transpose() *
                                          Du)   // Jacobian of force
            + k[2] * (outer(fvv, fvv) +
                      std::fmax(e_vv, 0.) * Dv.transpose() * Dv) +
            k[1] *
                ((outer(fuu, fvv)) + std::fmax(e_uu, 0.) * Dv.transpose() * Dv +
                 (outer(fvv, fuu)) +
...
void
Triangle::addConstraint(std::vector<Triplet> &tri, int &c_idx,
                        bool withWeight) {
    if (withWeight) {
        this->c_idx = c_idx;
    } else {
    ...
    for (int i = 0; i < 2; ++i) {
        for (int dim = 0; dim < 3; dim++) {
            tri.emplace_back(c_idx + dim + 3 * i, p0()->idx * 3 + dim,
                             -weightUsed *
                                 (inv_deltaUV(0, i) + inv_deltaUV(1, i)));
...
VecXd
Triangle::project(const VecXd &x_vec) const {
...
Mat3x2d
Triangle::projectToManifold(const VecXd &x_vec) const {
...
Mat6x9d
Triangle::projectToManifoldBackward(const VecXd &x_vec) const {
...
void
Triangle::projectBackwardPrecompute(const VecXd &x_vec) {
    newF = projectToManifoldBackward(x_vec);
    newF *= constrainWeightSqrt;
}

void
Triangle::projectBackward(const VecXd &x_vec, TripleVector &triplets) {
...
Triangle::Triangle(int p0_idx, int p1_idx, int p2_idx,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        double angle =
            ...
            180.0 / 3.14159265;
        if (angle < 20) {
            std::printf("WARNING: MESH TOPOLOGY: angle between point %d-%d-%d "
                        "is %.2f degree\n",
                        id1, id2, id3, angle);
        }
    }
